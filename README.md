...
1. HOST/SERVER reachability
2. internet connection
3. network interface Card available
4. To test DNS issues or IP exists
5. RTT and Gaming usage.
...
what is `ping` level in OSI network arch?

ICMP of course network layer

 ![image-20220630181825937](https://raw.githubusercontent.com/Juminiy/upload/main/img/image-20220630181825937.png)

Is `ping` permitted to be proxied ?

Never, it is located on network layer, the modern proxy way trojan,SOCK5,VMESS are application protocols,so in China Mainland we can never ping google,yahoo



how's it work?

send ICMP INternet COntrol Message Protocol echo to request to a specified interface on the network and waiting for a reply , RTT or Latency?



how to realize?

first ICMP signal and cli is enough.



操作系统原生的`ping`是怎么做的？

![image-20220630184622338](https://raw.githubusercontent.com/Juminiy/upload/main/img/image-20220630184622338.png)

IP ：直接发包 -> 失败 重试

域名：解析 解析失败，重试三次，返回错误 -> IP 发包 -> 成功？->未超过限定个数或未指定包个数，再次发送

发包：包大小，字节

ctrl + C 强制停止发送，结束此次CMD，打印所有统计信息



流程图为：

![image-20220701160201340](https://raw.githubusercontent.com/Juminiy/upload/main/img/image-20220701160201340.png)
...
OPTIONS
       ...
       -f
           Flood ping. For every ECHO_REQUEST sent a period “.” is printed, while for every ECHO_REPLY received a backspace is printed. This provides a rapid display of how many packets are being dropped.
       ...
       -i interval
           ...
           normally, or not to wait in flood mode. Only super-user may set interval to values less than 2 ms.
...
![image-20220701154317944](https://raw.githubusercontent.com/Juminiy/upload/main/img/image-20220701154317944.png)
...
```bash
git clone https://github.com/Juminiy/myping.git
git clone https://github.com/Juminiy/myping-view.git
cd myping && go mod tidy && go install
cd myping-view.git
```